...
public class HomeCommandImplV1 implements TodoCommand {
	 ...
	 * 이클래스를 상속받는 곳에서도 공용으로 사용할 수 있다
	 * 
	 * 이러한 코드가 상당히 이점은 있는데 간혹 메모리 leak(누수)가 발생할수 있다.
	 * 이럴때는 객체를 final로 선언해주는 것도 한가지 방법이다
	 * 
	 * 단, 객체를 final로 선언할 경우에는 
	 * 반드시 생성자에서 초기화를 해줘야한다 .
	 * 
	 */
	protected final TodoService tdService;
	protected static final Logger log = LoggerFactory.getLogger("TODO");
